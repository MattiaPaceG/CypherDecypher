...
class CipherToolGUI:
    ...
    def __init__(self, window):
        ...
        plaintext_label = tk.Label(self.encryption_frame, text="Plaintext:", font=("Arial", font_size_entries))
        plaintext_label.grid(row=1, column=0, pady=20)

        self.plaintext_entry = tk.Entry(self.encryption_frame, width=30, font=("Arial", font_size_entries))
        self.plaintext_entry.grid(row=1, column=1, padx= (0, 20), sticky=tk.W)

        caesar_shift_label = tk.Label(self.encryption_frame, text="Caesar Shift Value:", font=("Arial", font_size_entries))
        ...
        hexacode_label = tk.Label(self.encryption_frame, text="HexaCode Value:", font=("Arial", font_size_entries))
        ...
        after_caesar_shift_label = tk.Label(self.encryption_frame, text="After Caesar Shift:", font=("Arial", font_size_entries))
        after_caesar_shift_label.grid(row=5, column=0, padx=10)

        self.after_caesar_entry = tk.Entry(self.encryption_frame, width=30, font=("Arial", font_size_entries))
        self.after_caesar_entry.grid(row=5, column=1, padx=(0, 20), sticky=tk.W)
        self.after_caesar_entry.config(state="disabled")

        after_hexacode_label = tk.Label(self.encryption_frame, text="After HexaCode:", font=("Arial", font_size_entries))
        after_hexacode_label.grid(row=6, column=0, padx=10, pady=15)

        self.after_hexacode_entry = tk.Entry(self.encryption_frame, width=30, font=("Arial", font_size_entries))
        ...
        encrypt_clear_button.grid(row=7, column = 1, columnspan=1, pady=10, sticky=tk.W)

        copy_encrypted__button = tk.Button(self.encryption_frame, text="Copy Encryption", command=self.copy_encrypt, font=("Arial", 18))
        copy_encrypted__button.grid(row=7, column = 1, columnspan=1, pady=10, padx = (200,0), sticky=tk.W)
        ...
        ciphertext_label = tk.Label(self.decryption_frame, text="Ciphertext:", font=("Arial", font_size_entries))
        ciphertext_label.grid(row=1, column=0, pady=20)

        self.ciphertext_entry = tk.Entry(self.decryption_frame, width=30, font=("Arial", font_size_entries))
        self.ciphertext_entry.grid(row=1, column=1, padx= (0, 20), sticky=tk.W)

        reverse_caesar_shift_label = tk.Label(self.decryption_frame, text="Caesar Shift Value:", font=("Arial", font_size_entries))
        ...
        reverse_hexacode_shift_label = tk.Label(self.decryption_frame, text="HexaCode Value:", font=("Arial", font_size_entries))
        ...
        after_reverse_hexacode_label = tk.Label(self.decryption_frame, text="Reverse HexaCode:", font=("Arial", font_size_entries))
        after_reverse_hexacode_label.grid(row=5, column=0, padx=10)

        self.after_reverse_hexacode_entry = tk.Entry(self.decryption_frame, width=30, font=("Arial", font_size_entries))
        ...
        after_reverse_caesar_shift_label = tk.Label(self.decryption_frame, text="Reverse Caesar Shift:", font=("Arial", font_size_entries))
        after_reverse_caesar_shift_label.grid(row=6, column=0, padx=10, pady=15)

        self.after_reverse_caesar_entry = tk.Entry(self.decryption_frame, width=30, font=("Arial", font_size_entries))
        ...
        decrypt_clear_button = tk.Button(self.decryption_frame, text="Clear Section", command=self.clear_decrypt, font=("Arial", font_size_entries))
    ...
    def encrypt(self):
        ...
        try:
            ...
            Is_valid = self.cypdec.check_input(text_to_encrypt, shift,hexa)

            if (Is_valid[0] == True):
                #SEND TO ENCRYPTION
                encrypted = self.cypdec.encrypt(text_to_encrypt, shift, hexa)
                self.set_text(self.after_caesar_entry, encrypted[0], mode=1)
                self.set_text(self.after_hexacode_entry, encrypted[1], mode=1)

            else:
                message = ""
                if Is_valid[1] == 1:
                    message = "Plaintext has some not valid characters"
                else:
                    message = "HexaCode value is too big"
    ...
    def decrypt(self):
        ...
        try:
            ...
            Is_valid = self.cypdec.check_input(text_to_decrypt, shift, hexa)

            if (Is_valid[0] == True):
                # SEND TO ENCRYPTION
                decrypted = self.cypdec.decrypt(text_to_decrypt, shift, hexa)
                self.set_text(self.after_reverse_caesar_entry, decrypted[0], mode=1)
                self.set_text(self.after_reverse_hexacode_entry, decrypted[1], mode=1)

            else:
                message = ""
                if Is_valid[1] == 1:
                    message = "CipherText has some not valid characters"
                else:
                    message = "HexaCode value is too big"
